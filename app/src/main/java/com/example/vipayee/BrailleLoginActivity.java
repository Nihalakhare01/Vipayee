...
public class BrailleLoginActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Fetch userId dynamically from Constants
        String userId = Constants.getUserUUID(this);
        Log.d(TAG, "User ID from Constants: " + userId);
        if (userId == null || userId.trim().isEmpty()) {
            speakMessage("User ID not found. Please register or log in first.");
            // Optionally redirect to registration or login screen
            handler.postDelayed(() -> {
    ...
    private void fetchUserPin(String userId) {
        ...
        Log.d(TAG, "Fetching PIN from: " + url);
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    Log.e("API_ERROR", "Failed to retrieve PIN. Response Code: " + response.code() + ", Error: ");
                    runOnUiThread(() -> speakMessage("Failed to retrieve PIN. Error: " ));
                }
            }
        });
    }
